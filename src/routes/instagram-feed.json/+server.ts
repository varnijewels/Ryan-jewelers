...
export const GET = async ({ fetch }) => {
	...
	if (!accessToken) return json({ items: [] }, { headers: CACHE })
	...
	try {
		...
		if (!profileResponse.ok) return json({ items: [] }, { headers: CACHE })
		const profile = await profileResponse.json()
		const userId = String(profile.user_id || profile.id || '')
		if (!userId || profile.username !== 'ryan.jewelers') return json({ items: [] }, { headers: CACHE })
		...
		if (!mediaResponse.ok) return json({ items: [] }, { headers: CACHE })
		...
		return json({ items: [] }, { headers: CACHE })
	}
}
