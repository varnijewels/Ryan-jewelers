...
import { applyClientFilters, clientFilterKeys, clientSorts } from '$lib/theme/ryans-jewels/product-filters.js'
...
export const load = async (event: any) => {
	...
	if (result.products.totalPages > 1) {
		const pages = await Promise.all(Array.from({ length: result.products.totalPages - 1 }, async (_, index) => {
