...
<aside class="rj-account-sidebar" aria-label="My account navigation">
	<nav>
		{#each items as item}
			{#if item.divider}
				<hr />
...
<style>
	.rj-account-sidebar { box-sizing: border-box; width: 100%; height: 614px; padding: 25px; border: 1px solid #c2c2c2; border-radius: 6px; background: #fff; font-family: 'Lato', sans-serif; }
	...
	a img, button img { width: 24px; height: 24px; flex: 0 0 24px; }
	a.active { height: 40px; gap: 10px; justify-content: center; margin-block: -8px; padding: 8px 10px; border-radius: 3px; background: #cca646; color: #fff; }
