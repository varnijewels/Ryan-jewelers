<script lang="ts">
	...
	// `trigger` is optional: themes can supply their own toggle markup while
	// reusing all of the cart panel logic below. Omit it for the default button.
	interface Props {
		onClose?: any
		onContinueShopping?: any
		onRemoveCartItem?: any
		trigger?: Snippet<[{ toggle: () => void; qty: number; cart: any }]>
	}

	const { onClose, onContinueShopping, onRemoveCartItem, trigger }: Props = $props()
...
<div class="relative" role="navigation">
	{#if trigger}
		{@render trigger({ toggle: toggleCart, qty: cartState.cart?.qty || 0, cart: cartState.cart })}
	{:else}
		<button variant="ghost" size="icon" class="flex rounded-full px-2" aria-label="Toggle Cart" onclick={toggleCart}>
